...
import {
	...
	ChevronRight,
	Camera,
	KeyRound, // Ikon tambahan untuk Password
	Star,
} from "lucide-react";
...
export default function Profile() {
	...
	const accountMenu = [
		{ id: 1, label: "Poin Hadiah", icon: Gift, color: "text-amber-500", bg: "bg-amber-50", path: "/customer/reward-points" },
		{ id: 2, label: "Edit Profil", icon: User, color: "text-blue-500", bg: "bg-blue-50", path: "/customer/edit-profile" },
		{ id: 3, label: "Riwayat Transaksi", icon: FileText, color: "text-emerald-500", bg: "bg-emerald-50", path: "/customer/riwayat" },
		// { id: 4, label: "Bahasa", icon: Languages, color: "text-indigo-500", bg: "bg-indigo-50" },
	...
	// Ambil skor reputasi customer sendiri (rata-rata rating dari mitra),
	// supaya customer tahu skornya sendiri - sama seperti fitur di aplikasi
	// ride-hailing lain.
	useEffect(() => {
		if (!user.id) return;

		axios
			.get(`/customers/${user.id}/reputation`)
			.then((res) => setRatingStats(res.data))
			.catch((err) => console.error("Gagal memuat skor Anda:", err));
	}, [user.id]);
		...
		<CustomerLayout>
			<div className="w-full max-w-screen-xl mx-auto px-4 py-6">
				...
				<div className="flex items-center gap-4 mb-8 lg:hidden">
					...
					<h1 className="text-xl font-black text-indigo-900">Profil</h1>
				</div>
				...
				<div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
								...
								{ratingStats && (
									<div className="flex items-center justify-center mt-4">
										<div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/10 rounded-xl border border-white/20">
											...
											{ratingStats.average_rating && <span className="text-[10px] text-indigo-200 font-bold">({ratingStats.total_reviews} rating dari mitra)</span>}
										</div>
									</div>
								)}
							</div>
					...
					<div className="lg:col-span-8 space-y-6">
						...
						<div className="bg-white rounded-[40px] p-8 shadow-sm border border-gray-100">
							<h3 className="text-xs font-black text-gray-300 uppercase tracking-[0.2em] mb-8">Akun</h3>
