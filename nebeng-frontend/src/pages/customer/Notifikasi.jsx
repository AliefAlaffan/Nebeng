...
import { ChevronLeft, Bell, Trash2, CheckCheck, Clock, Tag, ShoppingBag, Wallet, Car, ShieldCheck, Star, Gift, Loader2 } from "lucide-react";
...
export default function Notifikasi() {
	...
	const filters = ["Semua", "order", "payment", "trip", "verification", "rating", "reward", "system"];
	const filterLabels = {
		Semua: "Semua",
		order: "Pesanan",
		payment: "Pembayaran",
	...
	const getNotificationStyle = (category) => {
		switch (category) {
			...
			case "reward":
				return { icon: Gift, color: "text-pink-500", bg: "bg-pink-50" };
			default:
			...
			<div className="w-full max-w-screen-xl mx-auto px-4 py-6">
				...
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
					<div className="flex items-center gap-4">
						<button onClick={() => navigate(-1)} className="p-2 bg-white rounded-xl shadow-sm border border-gray-100 lg:hidden">
					...
					<div className="lg:col-span-9 space-y-4">
						{isLoading ? (
							...
							<div className="bg-white rounded-[40px] p-20 text-center border-2 border-dashed border-gray-100 flex flex-col items-center">
								...
								<p className="text-sm text-gray-400 max-w-xs mx-auto">Notifikasi tentang perjalanan, pembayaran, dan sistem akan muncul di sini.</p>
							</div>
						)}
					</div>
				</div>
