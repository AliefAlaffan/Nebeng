...
export default function NebengMotor() {
	...
	const handleNext = () => {
		...
		navigate(`/customer/order-motor?origin_id=${selectedOrigin.id}&destination_id=${selectedDestination.id}&date=${date}`);
	};
		...
		<CustomerLayout>
			<div className="w-full max-w-screen-xl mx-auto px-4 py-6 pb-32">
				...
				<div className="flex items-center gap-4 mb-8">
					...
					<h1 className="text-2xl font-black text-indigo-900 tracking-tight">Nebeng Motor</h1>
				</div>
				...
				<div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
					...
					{/* SISI KANAN: Histori Alamat (5 Kolom) */}
					{/* <div className="lg:col-span-5">
						<div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 h-full">
							<div className="flex items-center gap-3 mb-8">
								...
								<h3 className="font-black text-gray-800 tracking-tight">Histori Alamat</h3>
							</div>

							<div className="space-y-4">
								{historyAddresses.map((addr) => (
									<div key={addr.id} onClick={() => selectHistory(addr.detail)} className="flex gap-4 p-5 rounded-2xl hover:bg-gray-50 transition-all border border-transparent hover:border-gray-100 group cursor-pointer">
										...
										<div className="flex-1">
											<p className="text-sm font-bold text-gray-800 mb-1">{addr.title}</p>
											<p className="text-[11px] leading-relaxed text-gray-400 font-medium">{addr.detail}</p>
										</div>
									</div>
								))}
							</div> */}

					{/* Placeholder jika histori kosong */}
					{/* {historyAddresses.length === 0 && (
								<div className="flex flex-col items-center justify-center py-20 text-center">
									<MapPin size={48} className="text-gray-100 mb-4" />
									<p className="text-sm font-bold text-gray-300">Belum ada riwayat alamat</p>
								</div>
							)}
						</div>
					</div>*/}
