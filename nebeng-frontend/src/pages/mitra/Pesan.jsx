...
export default function Pesan() {
	...
	const quickReplies = ["Order Anda sudah kami terima.", "Order sedang kami proses.", "Kami sedang menuju lokasi.", "Order telah selesai. Terima kasih.", "Apakah ada tambahan permintaan?"];
					...
					<div className={`flex-1 flex flex-col bg-gray-50/30 transition-all duration-500 ${!selectedChat ? "hidden lg:flex items-center justify-center" : "flex"}`}>
						{selectedChat ? (
							<>
								...
								{/* Quick Reply Mitra */}
								<div className="px-6 py-3 bg-white border-t border-gray-100 flex gap-2 flex-wrap">
									{quickReplies.map((text, i) => (
										<button key={i} onClick={() => setInputMessage(text)} className="text-xs px-3 py-1 rounded-full bg-indigo-50 text-indigo-700 hover:bg-indigo-100 transition">
											{text}
										</button>
									))}
								</div>
