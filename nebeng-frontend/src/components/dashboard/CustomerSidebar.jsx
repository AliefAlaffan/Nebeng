...
export default function CustomerSidebar({ sidebarOpen, setSidebarOpen }) {
	...
	const mainMenus = [
		{ name: "Beranda", path: "/customer/dashboard", icon: Home },
		{ name: "Riwayat", path: "/customer/riwayat", icon: Clock },
		{ name: "Pesan", path: "/customer/pesan", icon: MessageSquare },
			...
			>
				...
				<div className="p-8 relative">
					<div className="flex items-center gap-3 group cursor-pointer">
						<div className="p-2.5 bg-linear-to-br from-sky-400 to-indigo-500 rounded-2xl shadow-lg group-hover:rotate-12 transition-transform duration-300">
				...
				<div className="p-6 space-y-4">
					<div className="flex items-center gap-3 p-3 bg-black/20 rounded-2xl border border-white/5">
						...
						<div className="min-w-0 text-left">
							...
							<p className="text-[10px] text-emerald-400 font-bold uppercase">{user?.role || "Customer"}</p>
						</div>
					</div>
						...
						>
							{({ isActive }) => (
								<>
									...
									{isActive && <div className="absolute -top-2.5 w-12 h-1 bg-indigo-900 rounded-full animate-in slide-in-from-top-1" />}
