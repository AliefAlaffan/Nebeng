...
import { ChevronLeft, Navigation, ArrowUpRight, Search } from "lucide-react";
	...
	useEffect(() => {
		const fetchTrips = async () => {
			try {
				...
				// SAMA PERSIS DENGAN RIWAYAT MITRA
				const res = await fetch("http://127.0.0.1:8000/api/pos-mitra/trips", {
					headers: {
				...
				console.log("response trips", data);
				...
				const mapped = data.map((trip) => {
					...
					return {
						...
						hari: new Date(trip.departure_date).toLocaleDateString("id-ID", {
							weekday: "short",
						}),
			...
			<div className="w-full max-w-7xl mx-auto px-4 py-6">
				...
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
					<div className="flex items-center gap-4">
						...
						<h1 className="text-2xl font-black text-indigo-900 tracking-tight">Aktivitas Tebengan</h1>
					</div>
								...
								<div className="flex flex-col lg:flex-row justify-between gap-8">
									...
									<div className="lg:w-72 border-t lg:border-t-0 lg:border-l border-gray-50 pt-6 lg:pt-0 lg:pl-8 flex flex-col justify-between">
										<div>
											...
											<h2 className="text-2xl font-black text-indigo-900 tracking-tighter">
										...
										<button
											onClick={() => navigate(`/pos-mitra/detail-aktivitas/${item.id}`)}
